...
Open **`Services/Control/IPCService.qml`** and add a new handler block:
...
Check out these existing handlers in **`IPCService.qml`** for patterns:
...
**Method not found?**  
Double-check your target name and ensure **`IPCService.qml`** saved properly.
